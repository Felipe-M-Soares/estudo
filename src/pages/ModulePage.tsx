...
export function ModulePage({ progress, onToggleChecklist, onExerciseResult, onGameComplete, onSetCurrentModule }: ModulePageProps) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 lg:px-8">
      <Link to="/" className="mb-4 inline-flex items-center gap-1.5 text-sm text-base-400 hover:text-base-100">
      ...
      <div className="mb-6 animate-rise-in">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-mono text-xs uppercase tracking-wide text-base-400">Mês {mod.month} · Fase {mod.phase}</p>
            <h1 className="font-display text-2xl font-bold text-base-50 sm:text-3xl">{mod.title}</h1>
          </div>
        </div>
        <p className="mt-2 text-base-300">{mod.tagline}</p>
        <div className="mt-3 flex items-center gap-2">
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-base-700">
            <div className="h-full rounded-full bg-mint-400 transition-all duration-500" style={{ width: `${pct}%` }} />
          </div>
          <span className="mono-num text-xs text-base-400">{pct}%</span>
        </div>
      </div>
        ...
        {tabs.filter((t) => t.show).map((t) => (
          <button
            ...
            className={`shrink-0 rounded-full px-3.5 py-1.5 text-sm font-medium transition-colors ${
              tab === t.id ? 'bg-mint-400 text-base-950' : 'bg-base-800 text-base-300 hover:bg-base-700'
            }`}
          >
            {t.label}
          </button>
      ...
      {tab === 'conteudo' && (
        <div className="space-y-6 animate-rise-in">
          <p className="rounded-2xl border border-base-700 bg-base-850 p-4 text-base-200">{mod.intro}</p>
          {mod.lessons.map((lesson) => (
            <div key={lesson.id} className="rounded-2xl border border-base-700 bg-base-850 p-5">
              <h3 className="mb-3 font-display text-base font-bold text-mint-300">{lesson.heading}</h3>
              <MarkdownLite text={lesson.body} />
              {lesson.codeExample && (
                <pre className="mt-3 overflow-x-auto rounded-xl border border-base-700 bg-base-900 p-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {mod.resources.map((r) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-between gap-2 rounded-xl border border-base-600 px-3.5 py-2.5 text-sm text-base-200 transition-colors hover:border-mint-400/40 hover:bg-base-800"
                >
                  {r.label}
              ...
              {mod.games.map((g) => {
                ...
                return (
                  <button
                    ...
                    className="flex flex-col gap-1.5 rounded-2xl border border-base-700 bg-base-850 p-4 text-left transition-colors hover:border-mint-400/40"
                  >
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm transition-colors ${
                    done ? 'border-mint-400/40 bg-mint-900/20 text-mint-100' : 'border-base-600 bg-base-850 text-base-200 hover:border-base-500'
                  }`}
                >
      ...
      {tab === 'projeto' && mod.projectBrief && (
        <div className="space-y-4 animate-rise-in rounded-2xl border border-violet-400/30 bg-violet-500/10 p-5">
      ...
      <div className="mt-10 flex items-center justify-between border-t border-base-700 pt-6">
        {prev ? (
          <button onClick={() => navigate(`/modulo/${prev.id}`)} className="flex items-center gap-1.5 text-sm text-base-300 hover:text-base-100">
            <ArrowLeft size={14} /> {prev.title}
          </button>
        ) : <span />}
        {next && (
          <button onClick={() => navigate(`/modulo/${next.id}`)} className="flex items-center gap-1.5 text-sm font-semibold text-mint-400 hover:text-mint-300">
